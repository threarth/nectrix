...
Documento, plain text, occurrence, link, anchor, note editoriali, citazioni, stati Concept e indice di ricerca interessati da un salvataggio cambiano nella stessa transazione o non cambiano affatto.
...
Parent, ruolo strutturale e ordine editoriale non implicano Context o Tag ereditati e non modificano l'identità dei Concept.

**INV-DOC-09 — Split/move strutturale atomico**
Uno split/move tra Document opera su blocchi completi e trasferisce atomicamente ownership di occurrence, anchor, link sorgenti, note, citazioni e commenti, aggiornando i link entranti agli anchor spostati. O preserva tutti gli ID e la coerenza, oppure non modifica nulla.
...
Un'Occurrence appartiene a un solo Concept e a un solo Document. Riassegnare semanticamente un range crea una nuova Occurrence; soltanto uno split/move strutturale atomico può cambiare `document_id` preservando l'identità.

**INV-OCC-03 — Doppia rappresentazione coerente**  
Per un'Occurrence `active`, record DB e mark concordano su `occurrenceId`, `conceptId` e `documentId` implicito.
...
Cambiare Context a un Document o spostare un Context nella gerarchia non crea, fonde o rinomina Concept.
...
## F. Ipertesto, struttura ed export
