...
Le due cose restano distinte nel modello ma non nella resa: una `knowledgeOccurrence` ha un proprio fondo predefinito e si vede sempre, anche senza highlight, perché è l'unico modo per sapere leggendo che un testo è già conosciuto. Quel fondo copre l'eventuale evidenziazione sottostante: dove c'è un Concept o una Entity, la loro identità prevale su una decorazione libera. Concept ed Entity usano tinte diverse così il discriminator si legge senza aprire l'inspector. Il colore non è un dato di dominio, non viene persistito e non entra nel `document_json`: è resa dell'applicazione, derivata da `objectType`.
