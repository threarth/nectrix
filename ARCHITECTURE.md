...
Le due cose restano distinte nel modello ma non nella resa: una `knowledgeOccurrence` ha un proprio fondo predefinito e si vede sempre, anche senza highlight, perché è l'unico modo per sapere leggendo che un testo è già conosciuto. Proprio per questo un Concept o una Entity non si evidenziano: il comando Highlight resta disponibile solo dove sarebbe visibile, applicarlo a una selezione mista lo scrive soltanto fuori dalle occurrence, e creare una occurrence toglie l'evidenziazione dal suo intervallo. Così non esiste stato invisibile. Il fondo dell'occurrence copre comunque l'evidenziazione nei documenti salvati prima di questa regola, dove i due mark potevano convivere. Concept ed Entity usano tinte diverse così il discriminator si legge senza aprire l'inspector. Il colore non è un dato di dominio, non viene persistito e non entra nel `document_json`: è resa dell'applicazione, derivata da `objectType`.
